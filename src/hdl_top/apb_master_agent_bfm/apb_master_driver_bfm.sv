...
interface apb_master_driver_bfm (
                                 ...
                                 output logic [apb_global_pkg::NO_OF_SLAVES-1:0] pselx,

                                 output logic [apb_global_pkg::APB_DATA_WIDTH-1:0] pwdata,
                                 output logic [(apb_global_pkg::APB_DATA_WIDTH/8)-1:0] pstrb
                                );
  ...
  // Creating the handle for the proxy driver
  //------------------------------------------------------------------------------------------
  ...
  //------------------------------------------------------------------------------------------
  task drive_to_bfm(
    inout apb_transfer_char_s data_packet,
    input apb_transfer_cfg_s  cfg_packet
  );

    `uvm_info(
      name,
      $sformatf("data_packet=\n%p", data_packet),
      UVM_HIGH
    );
  ...
  task drive_idle_state();
    ...
    `uvm_info(
      "DEBUG_MSHA",
      $sformatf(
        "drive_apb_idle state = %0s and state = %0d",
        state.name(),
        state
      ),
      UVM_NONE
  ...
  task drive_setup_state(
    ...
    `uvm_info(
      name,
      $sformatf("DRIVING THE SETUP STATE"),
      UVM_HIGH
    );
    ...
    if (data_packet.pwrite == APB_WRITE) begin
    ...
    `uvm_info(
      "DEBUG_MSHA",
      $sformatf(
        "drive_apb_setup state = %0s and state = %0d",
        state.name(),
        state
      ),
      UVM_NONE
  ...
  //------------------------------------------------------------------------------------------
  // Task: waiting_in_access_state
  //
  // This task defines the accessing of data signals from master to slave
  // or vice versa
  //
  // Parameters:
  // data_packet - handle for apb_transfer_char_s
  ...
  task waiting_in_access_state(
    ...
    //   "DEBUG_MSHA",
    //   $sformatf(
    //     "wait_apb_access_state=%0d and state=%0d",
    //     state.name(),
    //     state
    //   ),
    //   UVM_NONE
  ...
  //
  // In this task, signals are waiting for pready to set to high
  // to transfer the data_packet
  //
  // Parameters:
  // data_packet - handle for apb_transfer_char_s
  //------------------------------------------------------------------------------------------
  task detect_wait_state(
    ...
    `uvm_info(
      name,
      $sformatf("DATA READY TO TRANSFER"),
      UVM_HIGH
    );


    if (data_packet.pwrite == APB_READ) begin
    ...
    // TODO(mshariff):
    // Add logic for making it work for ACCESS to SETUP phase
    ...
    //   "DEBUG_MSHA",
    //   $sformatf(
    //     "drive_apb_access state = %0s and state = %0d",
    //     state.name(),
    //     state
    //   ),
    //   UVM_NONE
